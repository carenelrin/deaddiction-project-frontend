...
import { FaMapMarkerAlt, FaCity, FaPhone, FaMapPin, FaBuilding, FaEnvelope } from "react-icons/fa";
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const response = await axios.get("https://deaddiction-project-backend.onrender.com/api/centre/profile", {
          headers: {
        ...
        setProfile(response.data.centre);
      } catch (error) {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center bg-gradient-to-br from-sky-100 to-white p-10 rounded-1xl max-w-8xl mx-auto mt-[90px] space-y-6 md:space-y-0 md:space-x-16 border border-sky-300 shadow-lg">
      <div className="flex-shrink-0 relative w-full md:w-96">
        <img
          src={profile.profilePhotoURL || "https://rehabcenters.com/wp-content/uploads/rehab-4.jpg"}
          alt="Hospital"
      ...
      <div className="ml-0 md:ml-16 mt-8 md:mt-0 flex-1">
        <h2 className="text-3xl font-semibold text-sky-700 mb-6">{profile.name || "Serenity Addiction Hospital"}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-5 text-gray-700">
          <p className="flex items-center text-lg">
            ...
            <span><strong>State:</strong> {profile.state || "California"}</span>
          </p>
          <p className="flex items-center text-lg">
            <FaCity className="text-sky-500 mr-4" />
            <span><strong>City:</strong> {profile.city || "Los Angeles"}</span>
          </p>
          <p className="flex items-center text-lg">
            <FaBuilding className="text-sky-500 mr-4" />
            <span><strong>Location:</strong> {profile.location || "Downtown District"}</span>
          </p>
          <p className="flex items-center text-lg">
            <FaPhone className="text-sky-500 mr-4" />
            <span><strong>Phone:</strong> {profile.phone || "+1 234-567-890"}</span>
          </p>
          <p className="flex items-center text-lg">
            <FaMapPin className="text-sky-500 mr-4" />
            <span><strong>PIN Code:</strong> {profile.pin || "90001"}</span>
          </p>
          <p className="flex items-center text-lg">
            <FaEnvelope className="text-sky-500 mr-4" />
            <span><strong>Email:</strong> {profile.email || "info@serenityhospital.com"}</span>
          </p>
        </div>
